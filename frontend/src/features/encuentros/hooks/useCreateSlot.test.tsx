...
import { useCreateSlot } from './useCreateSlot';
...
describe('useCreateSlot', () => {
  it('creates a slot with cohorte_id and resolves successfully', async () => {
    ...
    const { result } = renderHook(() => useCreateSlot(), { wrapper: makeWrapper() });
    ...
    await act(async () => {
      await result.current.mutateAsync({
        ...
        cant_semanas: 4,
        meet_url: 'https://meet.google.com/demo',
        video_url: null,
      });
    });
  ...
  it('surfaces backend validation errors', async () => {
    server.use(
      http.post('http://localhost:8000/api/encuentros/slots', () =>
        HttpResponse.json(
