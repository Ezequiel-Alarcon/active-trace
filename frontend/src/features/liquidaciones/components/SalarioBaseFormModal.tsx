...
import { z } from 'zod';
import { useCreateSalarioBase, useUpdateSalarioBase } from '../hooks/useGrillaSalarial';
import { Button } from '@/shared/ui';
import type { SalarioBase } from '../types/liquidaciones';
...
interface SalarioBaseFormModalProps {
  salario: SalarioBase | null;
  onClose: () => void;
}

export default function SalarioBaseFormModal({ salario, onClose }: SalarioBaseFormModalProps) {
  const create = useCreateSalarioBase();
  const update = useUpdateSalarioBase();
  const isEdit = Boolean(salario);

  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<FormData>({
    resolver: zodResolver(schema),
    defaultValues: salario
      ? { rol: salario.rol, importe: salario.importe, vigencia_desde: salario.vigencia_desde, vigencia_hasta: salario.vigencia_hasta }
      : { rol: '', importe: 0, vigencia_desde: '', vigencia_hasta: null },
  });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full mx-4">
        <h2 className="text-lg font-semibold mb-4">{isEdit ? 'Editar' : 'Agregar'} Salario Base</h2>
