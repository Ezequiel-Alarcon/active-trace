...
### RN-05 — Importación de padrón es versionada
Al importar un nuevo padrón de participantes para una materia, las instancias existentes pasan a inactivo/deleted y las nuevas se crean como activas, conservando el histórico completo del padrón. La operación no destruye datos previos: si un alumno estaba en el padrón anterior y no figura en el nuevo, su registro queda como inactivo (no se elimina).
...
Los plus son adicionales identificados por una **clave de categoría** (por ejemplo: una categoría que agrupa materias de programación) cruzada con el rol del docente. Las claves de categoría son un catálogo fijo definido por el programa/sistema, común a todas las instituciones; cada materia puede asociarse a una clave existente o quedar sin Plus. El valor del plus es configurable en la grilla. Si un docente tiene asignaciones en N comisiones de la misma categoría, acumula N veces el plus correspondiente.
...
No existe tope inicial de acumulación; cualquier tope futuro deberá incorporarse como regla versionada explícita.
