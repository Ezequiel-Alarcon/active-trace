# comunicacion-list-pending-lotes Specification

## Purpose
Define the canonical contract for listing grouped communication batches pending review for the current tenant. This specification documents the `GET /api/comunicaciones/lotes` endpoint, its RBAC requirement (`comunicacion:aprobar`), tenant-scoped filtering by optional `estado`, aggregated status counts by `lote_id`, and the supported response metadata fields only.
## Requirements
