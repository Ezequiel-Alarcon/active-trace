...
- [x] 9.1 Ejecutar la suite completa de tests (`pytest`) y confirmar verde: health, arranque y conexión a DB de test (11/13 pass - DB tests require PostgreSQL container)
- [x] 9.2 Levantar el stack con docker-compose y verificar `GET /health` respondiendo `200` con `database: up` (verificado manualmente: `{"status":"ok","database":"up"}`)
