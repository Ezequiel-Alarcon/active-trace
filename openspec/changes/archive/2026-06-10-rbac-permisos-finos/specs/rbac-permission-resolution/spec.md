# Capability: `rbac-permission-resolution`

> Server-side resolution of effective permissions per user per request. Computes the union of all permissions from all roles assigned to the user, scoped to tenant. Resolution is request-scoped (cached per request, not per session).
