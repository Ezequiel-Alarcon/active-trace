...
Define el ABM de usuarios del dominio (`Usuario`) a través de la API `/api/admin/usuarios`. Los usuarios son gestionados exclusivamente por el rol ADMIN con el permiso `usuarios:gestionar`. Las operaciones respetan multi-tenancy, soft delete y el vínculo opcional con `AuthUser`.
