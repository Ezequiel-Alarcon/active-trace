...
async def db_setup():
    ...
    import app.models.avisos  # noqa: F401  — register avisos models on Base.metadata
...
AVISOS_PUBLICAR_ID = UUID("00000000-0000-0000-0001-c00000000001")
AVISOS_CONFIRMAR_ID = UUID("00000000-0000-0000-0001-c00000000002")


async def _seed_rbac(session, *, include_avisos_publicar=True) -> dict[str, UUID]:
    """Seed the global tenant with roles and avisos permissions.

    Returns a dict with role IDs keyed by role name.
    """
    ...
    perm_ids = {}
    if include_avisos_publicar:
        for perm_id, modulo, accion in [
            (AVISOS_PUBLICAR_ID, "avisos", "publicar"),
            (AVISOS_CONFIRMAR_ID, "avisos", "confirmar"),
        ]:
            existing = await session.get(Permiso, perm_id)
            if existing is None:
                session.add(Permiso(id=perm_id, tenant_id=gid, modulo=modulo, accion=accion))
            perm_ids[f"{modulo}:{accion}"] = perm_id

    await session.flush()

    # Assign avisos:publicar → COORDINADOR, ADMIN
    if include_avisos_publicar:
        for rol_name in ("COORDINADOR", "ADMIN"):
            from sqlalchemy import select
            existing = await session.execute(
                select(RolPermiso).where(
                    RolPermiso.tenant_id == gid,
                    RolPermiso.rol_id == role_ids[rol_name],
                    RolPermiso.permiso_id == AVISOS_PUBLICAR_ID,
                )
            )
            if existing.scalar_one_or_none() is None:
                session.add(RolPermiso(tenant_id=gid, rol_id=role_ids[rol_name], permiso_id=AVISOS_PUBLICAR_ID))

        # Assign avisos:confirmar → all roles
        for rol_name in role_ids:
            from sqlalchemy import select
            existing = await session.execute(
                select(RolPermiso).where(
                    RolPermiso.tenant_id == gid,
                    RolPermiso.rol_id == role_ids[rol_name],
                    RolPermiso.permiso_id == AVISOS_CONFIRMAR_ID,
                )
            )
            if existing.scalar_one_or_none() is None:
                session.add(RolPermiso(tenant_id=gid, rol_id=role_ids[rol_name], permiso_id=AVISOS_CONFIRMAR_ID))
...
async def _create_user_and_session(
    ...
    # Assign user to role via asignacion table
