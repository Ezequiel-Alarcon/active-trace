"""Strict TDD for app.core.rate_limit (C-03 §1, D4, C-27 §3).

Spec contract:
- `RateLimiter` is a Protocol with `async check(key) -> bool` and `async record(key) -> None`.
...
async def test_rate_limiter_protocol_is_satisfied_by_default_impl() -> None:
...
async def test_check_within_limit_returns_true_when_below_threshold() -> None:
...
async def test_check_at_limit_returns_false() -> None:
...
async def test_check_does_not_consume_budget() -> None:
...
async def test_concurrent_record_does_not_exceed_limit_count() -> None:
...
async def test_check_does_not_evict_for_other_keys() -> None:
    """Eviction happens per-key, not across the whole store."""
    limiter = InMemorySlidingWindowRateLimiter(limit=1, window_seconds=1)
    limiter._now = lambda: 1000.0  # type: ignore[assignment]
    await limiter.record(("ip1", "a@b.com"))
    await limiter.record(("ip2", "a@b.com"))
    limiter._now = lambda: 1000.5  # type: ignore[assignment]
    # Both keys still within window
    assert await limiter.check(("ip1", "a@b.com")) is False
    assert await limiter.check(("ip2", "a@b.com")) is False
    # Move past window
    limiter._now = lambda: 1005.0  # type: ignore[assignment]
    assert await limiter.check(("ip1", "a@b.com")) is True
    assert await limiter.check(("ip2", "a@b.com")) is True


async def test_record_after_expiry_resets_budget() -> None:
    limiter = InMemorySlidingWindowRateLimiter(limit=1, window_seconds=1)
    limiter._now = lambda: 1000.0  # type: ignore[assignment]
    await limiter.record(("1.2.3.4", "a@b.com"))
    assert await limiter.check(("1.2.3.4", "a@b.com")) is False
